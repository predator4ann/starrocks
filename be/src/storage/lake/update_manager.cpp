...
Status UpdateManager::_write_segment_for_upsert(const TxnLogPB_OpWrite& op_write,
...

    {
        ...
        for (uint32_t cid : update_cids) {
            ...
            RETURN_IF_ERROR(
                    col_iter->fetch_values_by_rowid(insert_rowids.data(), insert_rowids.size(),
    ...
    for (uint32_t cid = 0; cid < tschema->num_columns(); ++cid) {
        ...
        if (has_default_value) {
            ...
            std::unique_ptr<DefaultValueColumnIterator> default_value_iter =
                    std::make_unique<DefaultValueColumnIterator>(true, default_value,
...
Status UpdateManager::_handle_upsert_index_conflicts(const TabletMetadataPtr& metadata, LakePrimaryIndex& index,
    ...
    RETURN_IF_ERROR(index.upsert(rowset_id + (uint32_t)new_rows_op.rowset().segments_size() - 1, 0,
                                    *pk_column, &new_deletes));
...
Status UpdateManager::_handle_column_upsert_mode(const TxnLogPB_OpWrite& op_write, int64_t txn_id,
    ...
    for (int i = 0; i < tschema->num_columns(); ++i) {
        if (tschema->column(i).is_auto_increment()) { ai_cid = i; break; }
    }
...
Status UpdateManager::_handle_delete_files(const TxnLogPB_OpWrite& op_write, int64_t txn_id,
                                            const TabletMetadataPtr& metadata, Tablet* tablet,
                                            LakePrimaryIndex& index, IndexEntry* index_entry,
...
Status UpdateManager::publish_column_mode_partial_update(const TxnLogPB_OpWrite& op_write, int64_t txn_id,
    ...
    RETURN_IF_ERROR(_handle_column_upsert_mode(op_write, txn_id, metadata, tablet, index, builder, base_version, rowset_id));

    // 2. handle delete files and generate delvecs for existing rssids only
    RETURN_IF_ERROR(_handle_delete_files(op_write, txn_id, metadata, tablet, index, index_entry, builder, base_version, del_rebuild_rssid, params));
...
static StatusOr<std::shared_ptr<Segment>> get_lake_dcg_segment(GetDeltaColumnContext& ctx, uint32_t ucid, int32_t* col_index,
    ...
    for (const auto& dcg : ctx.dcgs) {
        ...
        if (!column_file_result.ok()) {
            return Status::InternalError(fmt::format("DCG file not found for column {}: {}", 
                ucid, column_file_result.status().to_string()));
        }
        ...
        if (ctx.dcg_segments.count(column_file) == 0) {
            ...
            if (!dcg_segment_result.ok()) {
                return Status::InternalError(fmt::format("Failed to create DCG segment for column {}: {}", 
                    ucid, dcg_segment_result.status().to_string()));
            }
