...
Status UpdateManager::publish_column_mode_partial_update(const TxnLogPB_OpWrite& op_write, int64_t txn_id,
    ...
    if (index_entry != nullptr) {
        auto& index = index_entry->value();
        // 1. update primary index, handle delete files if present, and generate delvecs for existing rssids only
        PrimaryIndex::DeletesMap new_deletes;
        ...
        for (uint32_t del_id = 0; del_id < op_write.dels_size(); del_id++) {
            RETURN_IF_ERROR(state.load_delete(del_id, params));
            DCHECK(state.deletes(del_id) != nullptr);
            uint32_t del_rebuild_rssid = 0;
            for (const auto& rs : metadata->rowsets()) {
                if (rs.segments_size() > 0) {
                    del_rebuild_rssid = std::max<uint32_t>(del_rebuild_rssid, rs.id() + rs.segments_size() - 1);
                }
            }
            RETURN_IF_ERROR(index.erase(metadata, *state.deletes(del_id), &new_deletes, del_rebuild_rssid));
            _index_cache.update_object_size(index_entry, index.memory_usage());
        ...
        // 2. generate delvec and write to meta
    ...
    // 3. handle inserted rows: for COLUMN_UPSERT_MODE, build full segments with only inserted rows and append to meta
