...
Status UpdateManager::_handle_column_upsert_mode(const TxnLogPB_OpWrite& op_write, int64_t txn_id,
    ...
    for (uint32_t seg = 0; seg < op_write.rowset().segments_size(); ++seg) {
        ...
        if (!insert_rowids.empty()) {
            ...
            // 2. Each batch creates a new DCG (Delta Column Group) segment
            // 3. Accumulate all PKs in a single pk_column for efficiency
            // 4. Upsert PKs for all segments together to handle cross-segment conflicts
