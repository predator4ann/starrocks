...
Status KafkaProducer::init_config() {
    ...
    if (rd_kafka_conf_set(_conf, "bootstrap.servers", config::cdc_kafka_brokers.c_str(), errstr, sizeof(errstr)) !=
        RD_KAFKA_CONF_OK) {
        return Status::InternalError(strings::Substitute("Failed to set bootstrap.servers: $0", errstr));
    }

    // Set compression
    if (rd_kafka_conf_set(_conf, "compression.type", config::cdc_kafka_compression_type.c_str(), errstr,
                          sizeof(errstr)) != RD_KAFKA_CONF_OK) {
        return Status::InternalError(strings::Substitute("Failed to set compression.type: $0", errstr));
    }

    // Set acks
    if (rd_kafka_conf_set(_conf, "acks", config::cdc_kafka_acks.c_str(), errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
    ...
    if (rd_kafka_conf_set(_conf, "retries", retries_str.c_str(), errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
    ...
    if (rd_kafka_conf_set(_conf, "batch.size", batch_size_str.c_str(), errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
    ...
    if (rd_kafka_conf_set(_conf, "linger.ms", linger_str.c_str(), errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
    ...
    if (rd_kafka_conf_set(_conf, "message.timeout.ms", timeout_str.c_str(), errstr, sizeof(errstr)) !=
        RD_KAFKA_CONF_OK) {
    ...
    if (security_protocol == "sasl_plaintext" || security_protocol == "sasl_ssl") {
        if (rd_kafka_conf_set(_conf, "sasl.mechanism", config::cdc_kafka_sasl_mechanism.c_str(), errstr,
                              sizeof(errstr)) != RD_KAFKA_CONF_OK) {
        ...
        if (!config::cdc_kafka_sasl_username.empty()) {
            if (rd_kafka_conf_set(_conf, "sasl.username", config::cdc_kafka_sasl_username.c_str(), errstr,
                                  sizeof(errstr)) != RD_KAFKA_CONF_OK) {
        ...
        if (!config::cdc_kafka_sasl_password.empty()) {
            if (rd_kafka_conf_set(_conf, "sasl.password", config::cdc_kafka_sasl_password.c_str(), errstr,
                                  sizeof(errstr)) != RD_KAFKA_CONF_OK) {
    ...
    // Set client ID
    if (rd_kafka_conf_set(_conf, "client.id", "starrocks-cdc-producer", errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
...
Status KafkaProducer::send_sync(const std::string& topic, const std::string& key, const std::string& message,
                                int timeout_ms) {
    if (!_initialized.load()) {
        ...
        // Check completion without holding the poll lock to avoid deadlocks
        {
            ...
            if (sync_ctx->completed) {
                ...
                return Status::InternalError(
                        strings::Substitute("Kafka message delivery failed: $0", rd_kafka_err2str(sync_ctx->error)));
            }
        }
...
void KafkaProducer::delivery_report_cb(rd_kafka_t* rk, const rd_kafka_message_t* rkmessage, void* opaque) {
    ...
    if (rkmessage->err != RD_KAFKA_RESP_ERR_NO_ERROR) {
        ...
        VLOG(2) << "Kafka message delivered successfully to topic: " << rd_kafka_topic_name(rkmessage->rkt)
                << " partition: " << rkmessage->partition << " offset: " << rkmessage->offset;
    }
}
